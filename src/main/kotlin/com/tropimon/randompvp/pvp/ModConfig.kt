...
// config/randompvp-panels.properties (chargées au premier accès, écrites au drop).
...
object ModConfig {
    ...
    private fun fichier() =
        FabricLoader.getInstance().configDir.resolve("randompvp-panels.properties")
    ...
    private fun sauvegarder() {
        try {
            ...
            Files.newOutputStream(fichier()).use { p.store(it, "RandomPvp - positions des panneaux PvP") }
        } catch (_: Exception) {
        }
    }
    ...
    fun accentColor(): Int = 0xFFE8B84B.toInt()  // or RandomPvp
