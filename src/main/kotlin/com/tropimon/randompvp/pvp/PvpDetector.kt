...
// Seules modifications : package, logger, enregistrement dans RandomPvpClient.
package com.tropimon.randompvp.pvp
...
object PvpDetector {
    ...
    fun register() {
        ScreenEvents.AFTER_INIT.register afterInit@{ _, screen, _, _ ->
            ...
            com.tropimon.randompvp.RandomPvpClient.LOGGER.info("[PvP] Screen detected: \"$title\"")
        ...
        com.tropimon.randompvp.RandomPvpClient.LOGGER.info("[PvP] Detector registered")
    }
    ...
    private fun scanScreen(screen: HandledScreen<*>, ranked: Boolean) {
        ...
        try {
            ...
            com.tropimon.randompvp.RandomPvpClient.LOGGER.info("[PvP] Player   (${detectedPlayer.size}): ${detectedPlayer.map { it.speciesId }}")
            com.tropimon.randompvp.RandomPvpClient.LOGGER.info("[PvP] Opponent (${detectedOpponent.size}): ${detectedOpponent.map { it.speciesId }}")
        } catch (e: Exception) {
            com.tropimon.randompvp.RandomPvpClient.LOGGER.warn("[PvP] Scan error: ${e.message}", e)
        }
    }
    ...
    private fun extractPvpPokemon(stack: ItemStack): PvpPokemon? {
        ...
        val speciesId   = resolveSpeciesId(pokemonName) ?: run {
            com.tropimon.randompvp.RandomPvpClient.LOGGER.warn("[PvP] Unresolved species from name: \"$pokemonName\"")
            return null
        }
    ...
    private fun extractFromCobblemonComponent(stack: ItemStack): PvpPokemon? {
        try {
            ...
            com.tropimon.randompvp.RandomPvpClient.LOGGER.debug("[PvP] Component parse error: ${e.message}")
            return null
        }
    }
    ...
    private fun getPokemonItemComponentType(): net.minecraft.component.ComponentType<*>? {
        if (!componentTypeLookupDone) {
            ...
            if (pokemonItemComponentType == null) {
                com.tropimon.randompvp.RandomPvpClient.LOGGER.warn("[PvP] cobblemon:pokemon_item component type not found in registry")
            }
        }
