...
public final class InferenceCoverageCheck {
    ...
    public static void verifier() {
        ...
        if (!manquantsTalents.isEmpty()) {
            RandomPvpClient.LOGGER.warn(
                "[RandomPvp] Talents implémentés mais absents des listes candidates "
        ...
        if (!manquantsObjets.isEmpty()) {
            RandomPvpClient.LOGGER.warn(
                "[RandomPvp] Objets implémentés mais absents des listes candidates "
