    ...
    public static void charger() {
        Thread t = new Thread(() -> {
            for (String url : URLS_ESSAI) {
                try {
                    RandomPvpClient.LOGGER.info("[RandomPvp] Chargement sets Smogon : {}", url);
                    ...
                    HttpRequest req = HttpRequest.newBuilder()
                        ...
                        .header("User-Agent", "RandomPvp/1.0 Cobblemon-Fabric-Mod")
                        .header("Accept", "application/json")
                        .GET()
                        .build();
                    HttpResponse<String> resp = client.send(req, HttpResponse.BodyHandlers.ofString());
                    if (resp.statusCode() != 200) {
                        RandomPvpClient.LOGGER.warn("[RandomPvp] HTTP {} pour {}", resp.statusCode(), url);
                        continue;
                    }
                    parser(resp.body());
                    charge = true;
                    RandomPvpClient.LOGGER.info("[RandomPvp] Sets Smogon chargés : {} Pokémon ({})", DONNEES.size(), url);
                    return;
                } catch (Exception e) {
                    RandomPvpClient.LOGGER.warn("[RandomPvp] Échec chargement {} : {}", url, e.getMessage());
                }
            }
            erreur = true;
            RandomPvpClient.LOGGER.warn("[RandomPvp] Impossible de charger les sets Smogon.");
        }, "RandomPvp-SmogonLoader");
        t.setDaemon(true);
        t.start();
    }
